...
def render_sets_with_return(dataset : ModelParams, iteration : int, pipeline : PipelineParams, skip_train : bool, skip_test : bool, segment : bool = False, target = 'scene', idx = 0, precomputed_mask = None):
    dataset.need_features = dataset.need_masks = False
    if segment:
        assert target == 'seg' or precomputed_mask is not None and "Segmentation only works with target 'seg' or precomputed_mask!"
    gaussians, feature_gaussians = None, None
    with torch.no_grad():
        ...
        print("original size: ", gaussians.get_xyz.shape)
        ...
        if not skip_test:
             render_set(dataset.model_path, "test", scene.loaded_iter, scene.getTestCameras(), gaussians, pipeline, background, target, precomputed_mask=precomputed_mask)
        print("segmented size: ", gaussians.get_xyz.shape)
        gaussians.save_ply(os.path.join(dataset.model_path, f'point_cloud/SAGA.ply'))
        return gaussians
