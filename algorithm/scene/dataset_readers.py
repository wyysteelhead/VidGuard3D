...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder, features_folder = None, masks_folder = None, sample_rate = 1.0, divide = False):
    cam_infos = []
    for idx, key in enumerate(cam_extrinsics):
        ...
        if divide:
            if not os.path.exists(os.path.join(os.path.dirname(images_folder), "images_div", os.path.basename(image_path))):  
                print(f"{image_name} not found in the folder. Skipping...")  
                continue
...
def readColmapSceneInfo(path, images, eval, llffhold=8, need_features=False, need_masks=False, sample_rate = 1.0, divide = False):
    try:
