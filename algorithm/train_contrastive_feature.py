...
def training(dataset, opt, pipe, iteration, divide):
    ...
    sample_rate = 1.0
    scene = Scene(dataset, gaussians, feature_gaussians, load_iteration=iteration, shuffle=False, target='contrastive_feature', mode='train', sample_rate=sample_rate, divide=divide)
    
    # if os.path.isfile(os.path.join(scene.model_path, "point_cloud/iteration_{}/".format(scene.loaded_iter) + "sam_proj.pt")):
    #     print("sam_proj.pt already exists, skipping train_contrastive_feature.py")
    #     return
...
if __name__ == "__main__":
    ...
    training(lp.extract(args), op.extract(args), pp.extract(args), args.iteration, args.divide)
